...
public class IterativeParallelism implements ListIP {
    ...
    private int restThreadElements;
    ...
    private <T> void preparation(int threads, List<? extends T> list) {
        ...
        restThreadElements = list.size() % numberThreads;
    ...
    private <T, U> void initTreads(List<? extends T> values, ArrayList<U> resultTreads, Function<Stream<? extends T>, U> threadFunction) {
        for (int i = 0; i < numberThreads; i++) {
            int finalI1 = i;
            int finalI = i * eachThreadElements + ((i < restThreadElements) ? i : restThreadElements);
            int finale = finalI + eachThreadElements + ((i < restThreadElements) ? 1 : 0);
            Thread thread = new Thread(() -> {
                resultTreads.set(finalI1, threadFunction.apply(values.subList(finalI, finale).stream()));
            });
            thread.start();
    ...
    private <T, U> U task(int threads, List<? extends T> values, Function<Stream<? extends T>, U> threadFunction, Function<Stream<? extends U>, U> resFunction) throws InterruptedException {
        ...
        /*if (parallelMapper != null) {
            ArrayList<Stream<? extends T>> lists = new ArrayList<>();
            for (int i = 0; i < numberThreads; i++) {
                int finalI = i;
                lists.add(values.subList(finalI * eachThreadElements, (finalI + 1) * eachThreadElements).stream());
            }
            if (restThreadElements) {
                lists.add(values.subList(numberThreads * eachThreadElements, values.size()).stream());
            }
            return resFunction.apply(parallelMapper.map(threadFunction, lists).stream());
        }*/
        //ArrayList<U> resultTreads = new ArrayList<>(Collections.nCopies(numberThreads + ((restThreadElements) ? 1 : 0), null));
        ArrayList<U> resultTreads = new ArrayList<>(Collections.nCopies(numberThreads, null));
